...
export default function Context(Component) {
  const TmpClass = props => {
    ...
    const showText = !computedInvalid && text && (
      <div className={classNames("ffc-help", formText)}>{text}</div>
    );
    const showError = computedInvalid && error && (
      <div className={classNames("ffc-error", formError)}>{error}</div>
    );
